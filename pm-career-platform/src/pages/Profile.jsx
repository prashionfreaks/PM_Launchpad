...
export default function Profile() {
  ...
  // Start timer when entering evaluation tab without results
  useEffect(() => {
    if (activeTab === 'evaluation' && !state.quizResults && !quizSubmitted) {
      setTimerActive(true);
    } else {
      setTimerActive(false);
    }
  }, [activeTab, state.quizResults, quizSubmitted]);
  ...
  useEffect(() => {
    if (timeLeft === 0 && timerActive && !quizSubmitted) {
      handleTimerExpiry();
    }
                  ...
                  return (
                    <div key={cat.id} className="skill-bar-row">
                      ...
                      <div className="skill-bar-container">
                        <div className="skill-bar-fill" style={{ width: `${result?.percentage || 0}%`, background: cat.color }} />
                      </div>
                      <span className="skill-pct">{result?.percentage || 0}%</span>
                    </div>
                  );
                })}
              </div>
                  ...
                  <div className="score-circle-container">
                    <div className="score-circle" style={{ '--score-color': getScoreColor(overallScore) }}>
                      <svg viewBox="0 0 120 120">
                        <circle cx="60" cy="60" r="54" fill="none" stroke="var(--score-track, #e5e7eb)" strokeWidth="8" />
                        <circle
