...
export default function PMLoader({ message = 'Launching your PM journey...' }) {
  ...
  useEffect(() => {
    const setPath = () => {
      ...
      // S-curve: bottom-left → sweeps high → dips low → arrives at PM flag (right-center)
      const path =
        `M ${w * 0.03},${h * 0.82} ` +
        `C ${w * 0.22},${h * 0.05} ${w * 0.78},${h * 0.9} ${w * 0.94},${h * 0.47}`;
      if (planeRef.current) {
        planeRef.current.style.offsetPath = `path('${path}')`;
      }
    };
    setPath();
  ...
  return (
    <div className="pml-overlay">
      ...
      {/* Flying plane — path set dynamically via JS */}
      <div className="pml-plane-wrap" ref={planeRef}>
